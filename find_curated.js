...
const searchWiki = async (term) => {
    const url = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrnamespace=6&gsrsearch=filetype:bitmap|${encodeURIComponent(term)}&gsrlimit=3&prop=imageinfo&iiprop=url&format=json&origin=*`;
    try {
...
(async () => {
    ...
    fs.writeFileSync('curated_images.json', JSON.stringify(results, null, 2));
    console.log("Done.");
})();
